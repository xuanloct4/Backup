...
static void mov_build_index(MOVContext *mov, AVStream *st)
{
    ...
    //printf("MOV BUILD INDEX CALLED\n");
    /*
    if (st->codec->codec_type == AVMEDIA_TYPE_AUDIO)
        printf("MOV BUILD INDEX: TYPE AUDIO\n");
    if (st->codec->codec_type == AVMEDIA_TYPE_VIDEO)
        printf("MOV BUILD INDEX: TYPE VIDEO\n");
     */
    ...
    if (!(st->codec->codec_type == AVMEDIA_TYPE_AUDIO &&
        ...
        for (i = 0; i < sc->chunk_count; i++) {
            ...
            while (chunk_samples > 0) {
                ...
                printf(mov->fc, "AVIndex stream %d, chunk %d, offset %"PRIx64", dts %"PRId64", "
...
/* edit list atom */
/*
    There is an issue with multiple empty lists that has not been solved
    for some time. In my project every 1 + n 'files' that come through 
    this method will have two empty lists. The issue I am seeing is that
    dts/pts times on the audio stream are bad. Specifically the first few
    packets of audio in a H.264 stream are old. They are from the previous
    'file'. This issue does not occur with video. 
    
    The simple fix in my case is to just drop these packets in the library
    driver code. Basically I ignore the error that compute_pkt_fields2
    returns up through the call stack. But this does cause a very small
    loss in audio
*/
static int mov_read_elst(MOVContext *c, ByteIOContext *pb, MOVAtom atom)
{
    ...
    int version = get_byte(pb); /* version */
    int flags = get_be24(pb); /* flags */
    edit_count = get_be32(pb); /* entries */
    
    //printf("Version: %d - Flags: %d - Count: %d\n", version, flags, edit_count);

    if((uint64_t)edit_count*12+8 > atom.size)
        return -1;

    int offsets[edit_count] ;
    for(i=0; i<edit_count; i++){
        int time;
        int duration = get_be32(pb); /* Track duration */
        time = get_be32(pb); /* Media time */
        float rate =  get_be32(pb); /* Media rate */
        //printf("ittr = %d - Duration: %d - Time: %d - rate: %f \n", i, duration, time, rate);
...
static AVIndexEntry *mov_find_next_sample(AVFormatContext *s, AVStream **st)
{
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        if (msc->pb && msc->current_sample < avst->nb_index_entries) {
            ...
            //TODO: This is a rescale here and then it will be rescaled in FFmpeg
            //int64_t dts = av_rescale(current_sample->timestamp, AV_TIME_BASE, msc->time_scale);
            
            // On subsequent buffers the timestamp is old. This must be an issue with the actuall file.
            // The only thing I can think of is the AVAssetWriter is not calculating the proper offset.
            // The sample times are monotonic going in. They are not comming out of here. Again this is
            // Only with the audio. Additionally the duration is often wrong (see  mov_read_packet)
            
            int64_t dts = current_sample->timestamp ;//av_rescale(current_sample->timestamp, AV_TIME_BASE, msc->time_scale);
            
            dprintf(s, "stream %d, sample %d, dts %"PRId64"\n", i, msc->current_sample, dts);
            //printf("stream %d, sample %d, dts %"PRId64"\n", i, msc->current_sample, dts);
...
//    printf("MOV READ PACKET\n");
 ...
 retry:
    ...
    if (sc->ctts_data) {
        ...
        //TODO:
        // This is where the issue is. Sometimes we get a negative duration
        // PTS and DTS on the second file show previous time stamps. Could it be Apples fault?
    ...
    //printf("stream %d, pts %"PRId64", dts %"PRId64", pos 0x%"PRIx64", duration %d\n", 
    //       pkt->stream_index, pkt->pts, pkt->dts, pkt->pos, pkt->duration);
    return 0;
}
