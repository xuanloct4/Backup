...
//    printf("Exiting read frame internal\n\n");
    return 0;
}
...
//        pkt->pts= st->cur_dts;
    ...
    //printf("st->index=%d  s->dts=%"PRId64"  pkt->dts=%"PRId64 "  pkt->pts=%"PRId64"\n" ,st->index, st->cur_dts, pkt->dts, pkt->pts);
    //Changed >= to >
    if(st->cur_dts && st->cur_dts != AV_NOPTS_VALUE && st->cur_dts > pkt->dts){
        av_log(s, AV_LOG_ERROR, "st:%d error, non monotone timestamps %"PRId64" >= %"PRId64"\n",
